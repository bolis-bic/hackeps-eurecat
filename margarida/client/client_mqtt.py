...
def _connect_client(
    ...
    while retries < max_retries:
        try:
            ...
            logging.error(f"Unable to resolve MQTT broker service hostname '{host}'. Retrying in 1 second...")
            retries += 1
            time.sleep(1)
    logging.error(f"Exceeded maximum number of connection retries '{max_retries}'")
    return False
...
def main():
    ...
    _connect_client(client, environ.get("BROKER_ADDRESS"), int(environ.get("BROKER_ADDRESS_PORT")))
